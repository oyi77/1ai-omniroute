 ...
 * Supports: priority, weighted, round-robin, random, least-used, cost-optimized, and quota-aware strategies
 */

import { checkFallbackError, formatRetryAfter, getProviderProfile, getAccountHealth } from "./accountFallback.ts";
    ...
    models.map(async (modelStr) => {
      ...
      try {
        ...
        if (!connections || connections.length === 0) {
          return { modelStr, health: 100 };
        }
        let totalHealth = 0;
        for (const conn of connections) {
          totalHealth += getAccountHealth(conn);
        }
        const avgHealth = totalHealth / connections.length;
        return { modelStr, health: avgHealth };
      } catch {
        return { modelStr, health: 100 };
      }
    })
 ...
 * Supports all 7 strategies: priority, weighted, round-robin, random, least-used, cost-optimized, quota-aware
...
export async function handleComboChat({
  ...
  const strategy = combo.strategy || "quota-aware";
  ...
  if (strategy === "auto") {
    ...
    log.info("COMBO", `Quota-aware ordering: healthiest first (${orderedModels[0]})`);
  } else {
    orderedModels = await sortModelsByQuota(orderedModels);
    log.info("COMBO", `Default (quota-aware): healthiest first (${orderedModels[0]})`);
  }
