...
export async function handleChatCore({
  ...
  // Handle 401/403 - always try token refresh
  if (
    ...
    if (newCredentials?.accessToken || newCredentials?.copilotToken) {
      log?.info?.("TOKEN", `${provider.toUpperCase()} | refreshed (${providerResponse.status})`);
      ...
      try {
        ...
        if (retryResult.response.ok) {
          ...
          log?.warn?.("TOKEN", `${provider.toUpperCase()} | refresh didn't help (${retryResult.response.status})`);
        }
      } catch (retryError) {
        log?.warn?.("TOKEN", `${provider.toUpperCase()} | retry after refresh failed`);
      }
    } else {
      log?.warn?.("TOKEN", `${provider.toUpperCase()} | refresh failed (${providerResponse.status})`);
    }
  }

  // Handle 502 ONLY for OAuth-based providers that have refresh tokens
  // API key-based providers should not retry on 502 (it's a server error, not auth)
  if (
    providerResponse.status === HTTP_STATUS.BAD_GATEWAY &&
    credentials.refreshToken  // Only OAuth providers have refresh tokens
  ) {
    log?.info?.("TOKEN", `${provider.toUpperCase()} | 502 detected, attempting token refresh`);
    ...
    if (newCredentials?.accessToken || newCredentials?.copilotToken) {
      log?.info?.("TOKEN", `${provider.toUpperCase()} | refreshed after 502, retrying`);
      ...
      // Retry with new credentials - but only once to prevent loops
      try {
        const retryResult = await executor.execute({
          model,
          body: translatedBody,
        ...
        if (retryResult.response.ok) {
          providerResponse = retryResult.response;
          providerUrl = retryResult.url;
          log?.info?.("TOKEN", `${provider.toUpperCase()} | 502 retry succeeded`);
        } else {
          // Don't retry again - return the error
          log?.warn?.("TOKEN", `${provider.toUpperCase()} | 502 retry still failed (${retryResult.response.status})`);
        }
      } catch (retryError) {
        log?.warn?.("TOKEN", `${provider.toUpperCase()} | 502 retry error: ${retryError?.message}`);
      }
    } else {
      log?.debug?.("TOKEN", `${provider.toUpperCase()} | 502 - no refresh needed or refresh failed`);
    }
  }
