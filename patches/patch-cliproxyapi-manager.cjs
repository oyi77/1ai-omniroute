...
module.exports = async function (omniroute) {
  ...
  if (code.includes("UpdateLogModal") && code.includes("updateModalOpen")) {
    omniroute.logger.info(
      "[patch] patch-cliproxyapi-manager: File already has modal integration, skipping.",
    );
    return;
  }
  ...
  omniroute.logger.info(
    "[patch] patch-cliproxyapi-manager: Legacy file detected. Consider updating omniroute-src to get the new modal-based UI.",
  );
};
